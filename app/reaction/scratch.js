...
(function(){
	...
	fontDummy.onerror = function(){
		console.log("font loaded, probably");
		loaded = true;
	};
	fontDummy.src = "../FontStuck-Extended.ttf";
	...
	$DSRG = function(text){
		...
		var lightning = function(life,x,y,xd,yd,xtend){
			...
			var nx, ny;
			nx = x + xd*Math.random()*5 + xtend*Math.random()*2;
			ny = y + yd*Math.random()*5;
			ctx.lineWidth = 2;
			ctx.beginPath();
			ctx.moveTo(x,y);
			ctx.lineTo(nx,ny);
		...
		var animate = function(){
			...
			if((t%8)<4){
				lightning(100,lx,textY-Math.random()*fs,0,Math.random()*2-1,Math.random()*4-2);
			}
		...
		gif.on('finished', function(blob) {
			$canvas.trigger("gif",$("<img>").attr("src",URL.createObjectURL(blob)));
		});

		return $canvas;//for now
	};
